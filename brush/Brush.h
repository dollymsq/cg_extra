...
public:
    ...
    virtual void paintOnCanvasTmp(int x, int y, Canvas2D* canvas, float *flag);

    // fix alpha blending
    virtual void fixAlphaBlendingF(Canvas2D* canvas, BGRA* canvas_ori, float *flag);

    // for smudge
    virtual void clearPaint() {}

    //calculate the distance between point (x,y) and centre of mask
    double DistanceToCenter(int rowcounter, int colcounter);
